/*
    MicroGPT Large-Scale Evolution Engine - GPU Version
    
    GPU-accelerated version of the large evolution system.
    Uses CUDA for matrix operations when available, falls back to CPU otherwise.
*/
...
impl Genome {
    ...
    fn evaluate(&mut self, id: usize, use_gpu: bool) {
        if self.evaluated {
            eprintln!("[eval] organism {} already evaluated (loss={:.4})", id, self.loss);
            return;
        }
        
        let gpu_indicator = if use_gpu { "[GPU]" } else { "[CPU]" };
...
fn experiment_filename() -> String {
    ...
    format!("experiments/evolve_gpu_{}.log", now.format("%Y%m%d_%H%M%S"))
}

// --- GPU Detection and Setup ---

fn setup_gpu() -> (bool, String) {
    #[cfg(feature = "gpu")]
    {
        match GpuAccelerator::new() {
            Ok(gpu) => {
                let info = format!("GPU: {}", gpu.gpu_name());
...
fn main() {
    ...
    // Setup GPU first
    let (use_gpu, gpu_info) = setup_gpu();
    ...
    log!(log_file, "=== MicroGPT Loss Evolution Engine (GPU-Enhanced) ===");
    ...
    log!(log_file, "Acceleration: {}", if use_gpu { "GPU" } else { "CPU" });
    log!(log_file, "");
    ...
    for gen in 0..params.num_generations {
        ...
        if gen % 10 == 0 || gen == params.num_generations - 1 {
            ...
            println!("Progress: {}/{} generations ({:.1}%) - ETA: {:.0}s - Best: {:.4} ({})", 
                ...
                eta, gen_best.loss, if use_gpu { "GPU" } else { "CPU" });
        }
    ...
    if use_gpu {
        ...
        println!("  - GPU acceleration enabled for matrix operations");

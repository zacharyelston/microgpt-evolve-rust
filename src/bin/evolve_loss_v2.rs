/*
    MicroGPT Evolution Engine v3 - Clean & Simple
    
    A streamlined evolutionary optimizer that:
    1. Starts from existing genome (or random)
    2. Creates variations around the best
    3. Only saves when genuinely better
    ...
    fn evaluate(&mut self) {
        ...
        match result {
            Ok(r) => {
                ...
                self.evaluated = true;
                println!("[eval] {} | loss={:.4} ({:.1}s)", 
                    self.desc(), self.loss, start.elapsed().as_secs_f64());
            }
            Err(_) => {
                self.loss = f64::MAX;
                self.evaluated = true;
                println!("[eval] {} | PANICKED", self.desc());
            }
        }
    }
...
fn main() {
    ...
    println!("=== MicroGPT Evolution Engine v3 ===");
